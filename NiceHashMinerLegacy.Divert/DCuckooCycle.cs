...
namespace NiceHashMinerLegacy.Divert
{
    public class DCuckooCycle
    {
        ...
        public static IntPtr CuckooCycleDivertStart(List<string> processIdList, int CurrentAlgorithmType, string MinerName, string strPlatform)
            {
            Divert.CuckooCycledivert_running = true;

            DivertIP_Proxy1 = variables.ProxyIP;
            DivertPort_Proxy1 = 3014;

            filter = "(!loopback && outbound ? (tcp.DstPort == 9505 || tcp.DstPort == 4040 || " +
                "tcp.DstPort == 3014)" +
                " : " +
                "(tcp.SrcPort == 9505 || tcp.SrcPort == 4040 || " +
                "tcp.SrcPort == 3014)" +
                ")";
        ...
        //Span<byte> packetData = null;
            ...
            do
            {
                ...
                {
nextCycle:
                    if (Divert.CuckooCycledivert_running)
                    {
                        readLen = 0;
                        modified = false;
                        ...
                        {
                            {
                                ...
                                Divert.CuckooCycledivert_running = false;
                        ...
                        if (addr.Direction == WinDivertDirection.Outbound )
                        {
                            ...
                            if (Divert.SwapOrder(parse_result.TcpHeader->DstPort) == 9505 ||
...
modifyData:
                           ...
                           !OwnerPID.Equals("-1"))
                        {
                            ...
                            //{"id":1,"method":"mining.subscribe","params":["GMiner/2.03",null,"ae-pool.beepool.org","9505"]}
                            if (PacketPayloadData.Contains("mining.subscribe"))
                            {
                                dynamic json = JsonConvert.DeserializeObject(PacketPayloadData);
                                json.@params[2] = "ae.2miners.com";
                                json.@params[3] = "400";
                                PacketPayloadData = JsonConvert.SerializeObject(json).Replace(" ", "") + (char)10;
                                goto changePayloadData;
                            }
                            //{"id":2,"method":"mining.authorize","params":["ak_v4cBSQhjh8gc49XMmrt1ELXJDA8U7sDZVKhLJiAzjPymVFgFQ","x"]}
                            //                                               ak_2f9AMwztStKs5roPmT592wTbUEeTyqRgYVZNrc5TyZfr94m7fM
                            if (PacketPayloadData.Contains("mining.authorize") &&
                                Divert.SwapOrder(parse_result.TcpHeader->DstPort) == 9505)
                            {
                                Helpers.ConsolePrint("WinDivertSharp", "GMiner login detected to ae-pool.beepool.org");
...
sendPacket:
            ...
            while (Divert.CuckooCycledivert_running);
